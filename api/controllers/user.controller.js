...
class UserController{
    ...
    async updateProfile(req,res,next){
        ...
        const {firstName,lastName,email,newPwd,confirmPwd,currentPwdVerify}=req.body
        ...
        try {
            ...
            if(avatar){
                ...
                const user=await userModel.findByIdAndUpdate(userId.toString(),{$set:{avatar:response.url}},{new:true})
                await user.save()
            }
            ...
            var updatedUser=await userModel.findByIdAndUpdate((userId),{$set:{firstName,lastName,email}},{new:true})
            // if(imageURL){
            ...
            if(currentPwdVerify && currentPwdVerify !=='0'){
                ...
                return res.status(201).send({msg:"User Profile Updated With Password",updatedUser})
            }
            await updatedUser.save()
            return res.status(201).send({msg:"User Profile Updated Without Password",updatedUser})
        } catch (error) {
            next(error)
        }
    }
