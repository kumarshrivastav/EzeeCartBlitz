  ...
  async createCheckoutSession(req, res, next) {
    ...
    try {
      ...
      const groupedProducts = products?.reduce((acc, product) => {
        const key = `${product?.id}-${product?.price}`;
        if (!acc[key]) {
      ...
      const lineItems = Object.values(groupedProducts).map((product) => ({
        price_data: {
          currency: "inr",
          product_data: {
            name: product.name,
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        shipping: {
          ...
          address: {
            line1: customerInfo?.customerLine1,
            line2: customerInfo?.customerLine2,
            city: customerInfo?.customerCity,
            state: customerInfo?.customerState,
